...
app.get('/api/content/:category', async (req, res) => {
    ...
    try {
        ...
        Object.keys(params).forEach(key => {
            url.searchParams ? url.searchParams.set(key, params[key]) : null;
        });
...
app.get('/api/details/:type/:id', async (req, res) => {
    ...
    try {
        const response = await axios.get(
            `https://api.themoviedb.org/3/${type}/${id}?api_key=${TMDB_API_KEY}&append_to_response=videos,images,credits,similar,watch/providers`
        );
        res.json({ success: true, data: response.data });
    } catch (error) {
...
app.get('/api/trailer/:type/:id', async (req, res) => {
    ...
    try {
        ...
        const youtubeResponse = await axios.get(
            `https://www.googleapis.com/youtube/v3/search?key=${YOUTUBE_API_KEY}&q=${encodeURIComponent(`${title} ${year} official trailer`)}&part=snippet&maxResults=3&type=video`
        );
...
app.get('/api/youtube/search', async (req, res) => {
    ...
    try {
        const response = await axios.get(
            `https://www.googleapis.com/youtube/v3/search?key=${YOUTUBE_API_KEY}&q=${encodeURIComponent(q)}&part=snippet&maxResults=${maxResults}&type=video&videoEmbeddable=true`
        );
...
app.get('/api/youtube/video/:id', async (req, res) => {
    ...
    try {
        const response = await axios.get(
            `https://www.googleapis.com/youtube/v3/videos?key=${YOUTUBE_API_KEY}&id=${id}&part=snippet,statistics,contentDetails`
        );
...
app.get('/api/similar/:type/:id', async (req, res) => {
    ...
    try {
        const response = await axios.get(
            `https://api.themoviedb.org/3/${type}/${id}/similar?api_key=${TMDB_API_KEY}&page=${page}`
        );
        res.json({ success: true, data: response.data });
    } catch (error) {
        res.json({ success: true, data: { results: [] } });
